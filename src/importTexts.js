...
const importText = () => {
  try {
    ...
    const config = JSON.parse(
      fs.readFileSync(path.resolve(dir, 'config.json'), 'utf8')
    );

    const scenarios = config.scenarios.filter(scenario =>
      fs.lstatSync(getScenarioPath(scenario, dir)).isDirectory()
    );
    ...
    for (const scenario of scenarios) {
      ...
      const agents = config.agents.filter(agent =>
        fs
          .lstatSync(path.resolve(scenarioPath, getAgentName(agent) + '.md'))
          .isFile()
      );

      console.info(
        `Checking following ${agents.length} agents of ${scenario.name}:`,
        agents.map(agent => getAgentName(agent))
      );
      ...
      for (const agent of agents) {
        ...
        const text = fs
          .readFileSync(path.resolve(scenarioPath, agentName + '.md'), 'utf8')
          .trim();

        if (typeof agent === 'string') {
          data[scenarioIndex].agents.push({
            name: agent,
            slug: agentName,
            text
          });
...
const getScenarioPath = (scenario, dir) =>
  path.resolve(dir, scenario.slug || scenario.name.toLowerCase());

const getAgentName = agent =>
  typeof agent === 'string'
    ? agent.toLowerCase()
    : agent.slug || agent.name.toLowerCase();
